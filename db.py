...
class Database:
    ...
    async def delete_worker(self, name: str) -> None:
        await self.conn.execute("DELETE FROM workers WHERE name=?", (name,))
        await self.conn.commit()
    ...
    async def get_unassigned_tasks(self, offset: int = 0, limit: int = 20) -> List[Tuple]:
        cursor = await self.conn.execute(
            "SELECT id, title FROM tasks WHERE worker_id IS NULL ORDER BY id LIMIT ? OFFSET ?",
            (limit, offset),
        )
    ...
    async def get_tasks_page(self, offset: int, limit: int) -> List[Tuple]:
        cursor = await self.conn.execute(
            """
            SELECT t.id, t.title, t.type, w.name, t.created_at
            FROM tasks t LEFT JOIN workers w ON t.worker_id = w.id
    ...
    async def worker_stats_period(self, days: int) -> List[Tuple[str, int]]:
        cursor = await self.conn.execute(
            """
            SELECT w.name, COUNT(t.id) as cnt
            FROM tasks t
